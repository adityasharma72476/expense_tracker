...
from .validation import AppValidationError, get_idempotency_key, normalize_expense_input
...
def create_app(data_file=DATA_FILE, public_directory=PUBLIC_DIR):
    ...
    @app.get("/expenses")
    async def get_expenses(category: str = "", sort: str = ""):
        if sort and sort != "date_desc":
            raise AppValidationError("sort must be omitted or set to date_desc.")

        expense_view = app.state.store.get_expense_view(category=category, sort=sort)
    ...
    async def create_expense(request: Request):
        ...
        result = app.state.store.create_expense(
            normalized_expense, idempotency_key=idempotency_key
        )
        ...
        return JSONResponse(
            ...
            content={"expense": result["expense"], "replayed": not result["created"]},
        )
